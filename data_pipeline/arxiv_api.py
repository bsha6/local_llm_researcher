...
DB_PATH = config["database"]["arxiv_db_path"]
...
def fetch_arxiv_paper_data(max_results: int = 5):
...
def download_arxiv_pdf(paper_id, pdf_url, cursor):
    """Downloads a paper's PDF, saves it to disk, and updates the database with the file path."""
    pdf_filename = os.path.join(SAVE_PATH, "arxiv", f"{paper_id}.pdf")
    # Store relative path instead of absolute
    relative_pdf_path = os.path.relpath(pdf_filename, SAVE_PATH)
...
def store_papers_in_db(papers: List[Dict[str, str]]):
    ...
    with DatabaseManager(DB_PATH) as cursor:
        for paper in papers:
            ...
            download_arxiv_pdf(paper["id"], paper["pdf_url"], cursor)
